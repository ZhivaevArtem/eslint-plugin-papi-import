    ...
    create(context) {
        ...
        return {
            ImportDeclaration(node) {
                ...
                if (
                    !importFile || importFile.startsWith(resolvePath("node_modules"))
                    || !sourceCodeExtensions.some((ext) => importFile.endsWith(`.${ext}`)) || !sourceCodeExtensions.some((ext) => containFile.endsWith(`.${ext}`))
                ) {
                    return;
                }
